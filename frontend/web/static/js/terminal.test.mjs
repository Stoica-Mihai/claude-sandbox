...
// --- Escape → PTY routing (window-capture) --------------------------------
// Regression for the "press Esc twice" bug: the physical Escape keydown lands
// on <body>, not xterm's textarea, so xterm never emits \x1b. init() installs
// a window-capture keydown handler that sends \x1b to the visible terminal.
import { init as initTerminal } from './terminal.js';

// Build a document/window where window keydown listeners are captured and a
// single visible terminal instance is registered with a send-spy socket.
function loadEscRouting({ dialogOpen = false, target = { tagName: 'BODY', className: 'grain' } } = {}) {
    const winHandlers = [];
    const hidden = false;
    globalThis.document = {
        ...
        getElementById: (id) => (id === 'c1' ? { classList: { contains: (c) => (c === 'hidden' ? hidden : false) } } : null),
    };
    globalThis.window = {
        addEventListener: (type, fn, capture) => winHandlers.push({ type, fn, capture }),
        removeEventListener() {},
    };

    for (const k of Object.keys(TerminalManager.instances)) delete TerminalManager.instances[k];
    const sent = [];
    TerminalManager.instances['t1'] = {
        containerId: 'c1',
        socket: { send: (bytes) => { sent.push(bytes); return true; } },
    };

    initTerminal();
    const kd = winHandlers.find((h) => h.type === 'keydown' && h.capture === true);
    const ev = { key: 'Escape', target, preventDefault() { this.pd = true; }, stopPropagation() { this.sp = true; } };
    kd.fn(ev);
    return { sent, ev };
}

test('Escape on <body> routes \\x1b to the visible terminal', () => {
    const { sent, ev } = loadEscRouting();
    assert.equal(sent.length, 1, 'one send');
    assert.deepEqual(Array.from(sent[0]), [0x1b], 'sent a lone ESC byte');
    assert.equal(ev.pd, true, 'default prevented');
    assert.equal(ev.sp, true, 'propagation stopped (xterm never double-sends)');
});

test('Escape is left alone while a modal dialog is open', () => {
    const { sent } = loadEscRouting({ dialogOpen: true });
    assert.equal(sent.length, 0, 'modal owns Escape');
});

test('Escape in a non-terminal input field is left alone', () => {
    const { sent } = loadEscRouting({ target: { tagName: 'INPUT', className: 'renameInput' } });
    assert.equal(sent.length, 0, 'rename/settings field keeps Escape');
});
