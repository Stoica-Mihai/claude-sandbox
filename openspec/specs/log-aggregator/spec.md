...
The frontend SHALL proxy the log API to `logd` at both `/api/logs` and `/api/logs/` (so the bare path is not routed to the backend catch-all), and SHALL reject every method — including GET — from a tunnel-originated request with HTTP 403, because logs may contain secrets.

#### Scenario: tunnel visitor is denied all log access
- **WHEN** a request to `/api/logs` (any method, including GET) originates from the share tunnel
