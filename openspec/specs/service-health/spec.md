...
The frontend SHALL proxy the status API to logd at both `/api/status` and `/api/status/` (so the bare path is not routed to the backend catch-all), and SHALL reject every method — including GET — from a tunnel-originated request with 403, because status reveals fleet topology.

#### Scenario: tunnel visitor is denied status
- **WHEN** a request to `/api/status` (any method) originates from the share tunnel
